...
type Config struct {
	// DBUri    string `env:"MONGODB_LOCAL_URI"`
...
func LoadConfig(path string) (config Config, err error) {
	...
	envFile := path + "/.env"
	err = loadEnvFile(envFile, &config)
	if err != nil {
...
func loadEnvFile(filePath string, config *Config) error {
	...
	return setFieldsFromMap(config, envVars)
}
