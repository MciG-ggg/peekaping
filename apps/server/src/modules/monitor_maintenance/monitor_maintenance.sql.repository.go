...
type sqlModel struct {
	bun.BaseModel `bun:"table:monitor_maintenances,alias:mm"`

	ID            string    `bun:"id,pk"`
	MonitorID     string    `bun:"monitor_id,notnull"`
	MaintenanceID string    `bun:"maintenance_id,notnull"`
...
func toDomainModelFromSQL(sm *sqlModel) *Model {
	return &Model{
		...
		MaintenanceID: sm.MaintenanceID,
...
func (r *SQLRepositoryImpl) DeleteByMaintenanceID(ctx context.Context, maintenanceID string) error {
	_, err := r.db.NewDelete().Model((*sqlModel)(nil)).Where("maintenance_id = ?", maintenanceID).Exec(ctx)
	return err
}
