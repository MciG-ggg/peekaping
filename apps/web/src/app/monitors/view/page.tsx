...
import ImportantNotificationsList from "./ImportantNotificationsList";
...
const MonitorPage = () => {
  ...
  const pauseMutation = useMutation({
    ...
    onError: (error) => {
      console.log("error", error);
      toast.error("Error pausing monitor");
    },
  });
    ...
    const handleHeartbeat = (newHeartbeat: HeartbeatModel) => {
      ...
      if (newHeartbeat.important) {
        ...
        queryClient.setQueryData(queryKey, (oldData: any) => {
          ...
          const flat = oldData.pages.flatMap((page: any) => page.data);
          const filtered = flat.filter((hb: any) => hb.id !== newHeartbeat.id);
  ...
  }, [socket, socketStatus, id, heartbeatsResponse]);
            ...
            <Card className="p-4 rounded-xl gap-1">
              ...
              <div
                className={clsx(
                  ...
                  lastHeartbeat?.status ? "text-green-400" : "text-red-400"
                )}
              >
                {lastHeartbeat?.status ? "Up" : "Down"}
              </div>
              {/* <div className="text-sm text-gray-400">
                Currently {lastHeartbeat?.status ? "up" : "down"} for{" "}
